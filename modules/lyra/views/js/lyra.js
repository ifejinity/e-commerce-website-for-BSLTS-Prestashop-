...
function lyraAddFranfinanceOption(first) {
    if (first) {
        $('#lyra_ffin_options_btn').hide();
        $('#lyra_ffin_options_table').show();
    }

    var timestamp = new Date().getTime();
    var rowTpl = $('#lyra_ffin_row_option').html();
    rowTpl = rowTpl.replace(/LYRA_FFIN_KEY/g, '' + timestamp);

    $(rowTpl).insertBefore('#lyra_ffin_option_add');
}

function lyraDeleteFranfinanceOption(key) {
    $('#lyra_ffin_option_' + key).remove();

    if ($('#lyra_ffin_options_table tbody tr').length === 1) {
        $('#lyra_ffin_options_btn').show();
        $('#lyra_ffin_options_table').hide();
        $('#lyra_ffin_options_table').append("<input type=\"hidden\" id=\"LYRA_FFIN_OPTIONS\" name=\"LYRA_FFIN_OPTIONS\" value=\"\">");
    }
}
...
function lyraAddExtraPaymentMeansOption(first) {
    if (first) {
        $('#lyra_extra_payment_means_options_btn').hide();
        $('#lyra_extra_payment_means_options_table').show();
        $('#LYRA_EXTRA_PAYMENT_MEANS').remove();
    }

    var timestamp = new Date().getTime();

    var rowTpl = $('#lyra_add_payment_means_row_option').html();
    rowTpl = rowTpl.replace(/LYRA_EXTRA_PAYMENT_MEANS_SCRIPT_KEY/g, '' + timestamp);

    $(rowTpl).insertBefore('#lyra_extra_payment_means_option_add');
}

function lyraDeleteExtraPaymentMeansOption(key) {
    $('#lyra_extra_payment_means_option_' + key).remove();

    if ($('#lyra_extra_payment_means_options_table tbody tr').length === 1) {
        $('#lyra_extra_payment_means_options_btn').show();
        $('#lyra_extra_payment_means_options_table').hide();
        $('#lyra_extra_payment_means_options_table').append("<input type=\"hidden\" id=\"LYRA_EXTRA_PAYMENT_MEANS\" name=\"LYRA_EXTRA_PAYMENT_MEANS\" value=\"\">");
    }
}
...
function lyraSepa1clickPaymentMenuDisplay(sepaMandateModeId) {
    var sepaMandateMode = $('#' + sepaMandateModeId).val();
    if (sepaMandateMode === 'REGISTER_PAY') {
        $('#LYRA_SEPA_1_CLICK_PAYMNT_MENU').show();
    } else {
        $('#LYRA_SEPA_1_CLICK_PAYMNT_MENU').hide();
    }
}