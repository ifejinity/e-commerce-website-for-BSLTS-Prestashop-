    ...
    public function postProcess()
    {
        ...
        switch ($type) {
            case 'standard':
                ...
                if ($payment->getEntryMode() === LyraTools::MODE_LOCAL_TYPE) {
                    $data['card_type'] = Tools::getValue('lyra_card_type');
                } elseif ($payment->getEntryMode() === LyraTools::MODE_IFRAME) {
            ...
            case 'franfinance':
                $payment = new LyraFranfinancePayment();

                $data['opt'] = Tools::getValue('lyra_ffin_option');

                $options = LyraFranfinancePayment::getAvailableOptions($cart);
                $option = ' (' . $options[$data['opt']]['count'] . ' x)';
                break;
            ...
            case 'sepa':
                ...
                // Payment by alias.
                if (Configuration::get('LYRA_SEPA_1_CLICK_PAYMNT') === 'True') {
                    $data['sepa_payment_by_identifier'] = Tools::getValue('lyra_sepa_payment_by_identifier', '0');
                }
        ...
        $this->context->smarty->assign('lyra_logo', $payment->getLogo());
