...
require_once(_PS_MODULE_DIR_ . 'lyra/autoload.php');
...
class Lyra extends PaymentModule
{
    ...
    public function install()
    {
        ...
        $installError = false;
        if (! defined('LYRA_MODULE_UPGRADE')) {
        ...
        $admin_config_params = LyraTools::getAdminParameters();

        if (defined('LYRA_MODULE_UPGRADE')) {
            $results = Db::getInstance()->executeS(
                'SELECT `name` FROM `' . _DB_PREFIX_ . "configuration` WHERE `name` LIKE 'LYRA_%' AND `name` NOT LIKE 'LYRA_OS_%'"
            );
        ...
        foreach ($admin_config_params as $param) {
            ...
            if (! defined('LYRA_MODULE_UPGRADE') || ! in_array($key, $already_installed_params)) {
                ...
                } elseif (defined('LYRA_TRANSIENT_DEFAULT')) {
                    $defaults = unserialize(LYRA_TRANSIENT_DEFAULT);
        ...
        if (defined('LYRA_MODULE_UPGRADE') && ! empty($already_installed_params)) {
        ...
        if (! Configuration::get('LYRA_OS_REFUNDED')) {
            // Create to validate payment order state.
            $name = array(
                'en' => 'Refunded with Lyra Collect',
                'fr' => 'Remboursé avec Lyra Collect',
                'de' => 'Rückerstattet mit Lyra Collect',
                'es' => 'Reembolsado con Lyra Collect'
            );

            $refund_state = new OrderState();
            $refund_state->name = LyraTools::convertIsoArrayToIdArray($name);
            ...
            if (! $refund_state->save() || ! Configuration::updateValue('LYRA_OS_REFUNDED', $refund_state->id)) {
                $this->logger->logWarning('Error while creating customized order status «Refunded with Lyra Collect».');

                $this->_errors[] = sprintf($this->l('Error while creating customized order status « %s ».'), sprintf($this->l('Refunded with %s'), 'Lyra Collect'));
                $installError = true;
            }

            // Add small icon to state.
            @copy(
                _PS_MODULE_DIR_ . 'lyra/views/img/os_refund.gif',
                _PS_IMG_DIR_ . 'os/' . Configuration::get('LYRA_OS_REFUNDED') . '.gif'
            );
        }
    ...
    public function getContent()
    {
        ...
        if (isset($this->context->cookie->lyraEmailSendMsg)) {
            $msg = $this->context->cookie->lyraEmailSendMsg;
            unset($this->context->cookie->lyraEmailSendMsg);
        }
        ...
        if (Tools::isSubmit('lyra_submit_admin_form')) {
            ...
            $msg .= '<br />';
        } else if (Tools::getValue('submitter') === 'lyra_send_support') {
            if (Tools::getValue('sender') && Tools::getValue('subject') && Tools::getValue('message')) {
                ...
                if ($this->sendEmail($email)) {
                   if (Tools::getValue('lyra_mail_origine') === 'order') {
                       // Display success message in Order details page.
                       $this->context->cookie->lyraMessageSuccessSent = $this->l('Thank you for contacting us. Your email has been successfully sent.');
                       die();
                   } else {
                    ...
                    if (Tools::getValue('lyra_mail_origine') === 'order') {
                        ...
                        $this->context->cookie->lyraMessageErrorSent = $this->l('An error has occurred. Your email was not sent.');
                        die();
                    } else {
                ...
                $this->context->cookie->lyraEmailSendMsg = $msg;
            } else {
                if (Tools::getValue('lyra_mail_origine') === 'order') {
                    // Display error message in Order details page.
                    $this->context->cookie->lyraMessageErrorSent = $this->l('Please make sure to configure all required fields.');
                    die();
                } else {
                    $this->context->cookie->lyraEmailSendMsg = $this->displayError($this->l('Please make sure to configure all required fields.'));
                }
            }
        }
    ...
    private function sendEmail($email)
    {
        return Mail::Send(
            ...
            'lyra', // Email template file to be use.
            $email['subject'], // Email subject.
            array(
                '{email}' => $email['sender'], // Sender email address.
                '{message}' => $email['message'] // Email content.
            ),
            LyraTools::getDefault('SUPPORT_EMAIL'), // Receiver email address.
            NULL, // Receiver name.
        ...
        foreach (LyraTools::getAdminParameters() as $param) {
            ...
            if (in_array($key, LyraTools::$multi_lang_fields)) {
                ...
                if (! is_array($value) || empty($value)) {
                    ...
                    $extra_cards = @unserialize(Configuration::get('LYRA_EXTRA_PAYMENT_MEANS'));
                ...
                $value = serialize($value);
            } elseif ($key === 'LYRA_EXTRA_PAYMENT_MEANS') {
                $used_cards = array_keys(LyraApi::getSupportedCardTypes());
                if (! is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
                ...
                // Update LYRA_OTHER_PAYMENT_MEANS (options containing deleted payment means should not appear).
                ...
                Configuration::updateValue('LYRA_OTHER_PAYMENT_MEANS', serialize($other_payment_means));
            ...
            } elseif ($key === 'LYRA_REST_SERVER_URL' || $key === 'LYRA_REST_JS_CLIENT_URL') {
    ...
    {
        $label = $this->l('Payment in 3 or 4 times Oney', 'lyraoney34payment');
        ...
        if (! $inside) {
            ...
            if (! Tools::getValue('LYRA_ONEY34_OPTIONS')) {
    ...
    private function renderForm()
    {
        ...
        if (LyraTools::$plugin_features['support']) {
        ...
        if ($controller instanceof OrderController || $controller instanceof OrderOpcController) {
            ...
            if ($standard->isAvailable($this->context->cart)) {
                if ($standard->isEmbedded()) {
                    ...
                    $html .= '<script src="' . Configuration::get('LYRA_REST_JS_CLIENT_URL') . 'js/krypton-client/V4.0/stable/kr-payment-form.min.js">
                              </script>' . "\n";

                    // Theme and plugins, should be loaded after the javascript library.
                    $rest_theme = Configuration::get('LYRA_STD_REST_THEME') ? Configuration::get('LYRA_STD_REST_THEME') : 'material';
                    $html .= '<link rel="stylesheet" href="' . Configuration::get('LYRA_REST_JS_CLIENT_URL') . 'js/krypton-client/V4.0/ext/' . $rest_theme . '-reset.css">
                              <script src="' . Configuration::get('LYRA_REST_JS_CLIENT_URL') . 'js/krypton-client/V4.0/ext/' . $rest_theme . '.js"></script>';
    ...
    public function hookPayment($params)
    {
        ...
        $franfinance = new LyraFranfinancePayment();
    ...
    public function hookPaymentOptions($params)
    {
        ...
        if ($franfinance->isAvailable($cart)) {
            ...
            if ($franfinance->hasForm()) {
                $this->context->smarty->assign($franfinance->getTplVars($cart));
                $form = $this->fetch('module:lyra/views/templates/hook/' . $franfinance->getTplName());
                $option->setForm($form);
            }
        ...
        if ($sepa->isAvailable($cart)) {
            ...
            if ($sepa->isOneClickActive()) {
                ...
                if ($isSepaValidAlias) {
                    $this->context->smarty->assign($sepa->getTplVars($cart));
                    $additionalForm = $this->fetch('module:lyra/views/templates/hook/payment_sepa_oneclick.tpl');
                    $option->setAdditionalInformation($additionalForm);
                    $option->setForm(
                        '<form action="' . $this->context->link->getModuleLink('lyra', 'redirect', array(), true) . '" method="post">
    ...
    public function hookPaymentReturn($params)
    {
        ...
        if (! $error) {
            $array['total_to_pay'] = LyraTools::formatPrice(
    ...
    public function hookDisplayAdminOrderTop($params)
    {
        ...
        if (isset($this->context->cookie->lyraMessageErrorSent)) {
            $this->get('session')->getFlashBag()->set('error', $this->context->cookie->lyraMessageErrorSent);
            unset($this->context->cookie->lyraMessageErrorSent);
            $this->context->cookie->write();

            Tools::redirectAdmin($url_admin_order);
        }

        if (isset($this->context->cookie->lyraMessageSuccessSent)) {
            $this->get('session')->getFlashBag()->set('success', $this->context->cookie->lyraMessageSuccessSent);
            unset($this->context->cookie->lyraMessageSuccessSent);
            $this->context->cookie->write();
    ...
    public function hookDisplayAdminOrder($params)
    {
        ...
        if (isset($this->context->cookie->lyraMessageErrorSent)) {
            $this->context->controller->errors[] = $this->context->cookie->lyraMessageErrorSent;
            unset($this->context->cookie->lyraMessageErrorSent);
        }

        if (isset($this->context->cookie->lyraMessageSuccessSent)) {
            $this->context->controller->confirmations[] = $this->context->cookie->lyraMessageSuccessSent;
            unset($this->context->cookie->lyraMessageSuccessSent);
        }
    ...
    {
        $template = _PS_MODULE_DIR_ . 'lyra/views/templates/admin/';
    ...
    private function displaySupportContactFromOrderDetails($order)
    {
        ...
        $tpl_vars = LyraHelperForm::getAdminFormContext();
        $tpl_vars['trans_id_title'] = $this->l('Transaction UUID : ');
        $tpl_vars['lyra_site_id'] = Configuration::get('LYRA_SITE_ID', null, $order->id_shop_group, $order->id_shop);
        $tpl_vars['lyra_mode'] = Configuration::get('LYRA_MODE', null, $order->id_shop_group, $order->id_shop);
        $tpl_vars['lyra_sign_algo'] = Configuration::get('LYRA_SIGN_ALGO', null, $order->id_shop_group, $order->id_shop);

        $card_data_mode = Configuration::get('LYRA_STD_CARD_DATA_MODE', null, $order->id_shop_group, $order->id_shop);
        $tpl_vars['lyra_std_card_data_mode'] = $card_data_mode ? $card_data_mode : '1';
        ...
        $tpl_vars['total_paid'] = LyraTools::formatPrice($order->total_paid, $order->id_currency, $this->context);
        $tpl_vars['total_products_wt'] = LyraTools::formatPrice($order->total_products_wt, $order->id_currency, $this->context);
        $tpl_vars['total_shipping'] = LyraTools::formatPrice($order->total_shipping, $order->id_currency, $this->context);
        $tpl_vars['total_discounts'] = LyraTools::formatPrice($order->total_discounts, $order->id_currency, $this->context);
        ...
        // Recover POST uri.
        $lyra_request_uri = $this->context->link->getAdminLink('AdminModules');
        $lyra_request_uri = str_replace('&token=', '&configure=lyra&token=', $lyra_request_uri);
        $tpl_vars['lyra_request_uri'] = $lyra_request_uri;
        $this->context->smarty->assign($tpl_vars);

        return $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'lyra/views/templates/admin/support_contact_from_order_details.tpl');
    }
    ...
    public function hookActionOrderStatusUpdate($params)
    {
        ...
        if (LyraTools::checkFormIpnValidity() || LyraTools::checkRestIpnValidity()) {
            return;
        }

        $managedRefundStatuses = array((int) Configuration::get('LYRA_OS_REFUNDED'), (int) Configuration::get('PS_OS_CANCELED'));
        ...
        // Update order status is manually changed to "Refunded with Lyra Collect or Canceled" (not by refund function).
        $this->context->cookie->lyraManualUpdateToManagedRefundStatuses = 'True';
        ...
        if (! $this->refund($order, $order->total_paid_real)) {
            if (Tools::isSubmit('token')) {
                ...
                if (isset($this->context->cookie->lyraRefundWarn)) {
                    $this->get('session')->getFlashBag()->set('warning', $this->context->cookie->lyraRefundWarn);
                    unset($this->context->cookie->lyraRefundWarn);
                }
        ...
        } elseif (! Tools::isSubmit('token') && isset($this->context->cookie->lyraRefundWarn)) {
            // Display warning to customer if any for Prestashop versions >= 1.7.7.
            $this->get('session')->getFlashBag()->set('warning', $this->context->cookie->lyraRefundWarn);
            unset($this->context->cookie->lyraRefundWarn);
        }
    ...
    public function hookActionOrderStatusPostUpdate($params)
    {
        ...
        if (isset($this->context->cookie->lyraManualUpdateToManagedRefundStatuses)
            && ($this->context->cookie->lyraManualUpdateToManagedRefundStatuses === 'True')) {
            unset($this->context->cookie->lyraManualUpdateToManagedRefundStatuses);
    ...
    {
        if (! Tools::isSubmit('doPartialRefundLyra')
            && ! Tools::isSubmit('doStandardRefundLyra')) {
            return;
        }
    ...
    private function refund($order, $amount)
    {
        ...
        if (! $this->getPrivateKey()) {
            ...
            $this->context->cookie->lyraRefundWarn = $this->l('Payment is refunded/canceled only in PrestaShop. Please, consider making necessary changes in Lyra Expert Back Office.');
            return true;
        }
    ...
    private function refundFromOneTransaction($order, $amountInCents, $transaction, $currency, $orderCurrency, $forceUpdateOrderStatus = null)
    {
        ...
        /** @var LyraRest $client */
        $client = new LyraRest(
            Configuration::get('LYRA_REST_SERVER_URL'),
        ...
        if ($transStatus === 'CAPTURED') { // Transaction captured, we can do refund.
            ...
            $isManualUpdateRefundStatus = isset($this->context->cookie->lyraManualUpdateToManagedRefundStatuses) && ($this->context->cookie->lyraManualUpdateToManagedRefundStatuses === 'True');
            if (! $isManualUpdateRefundStatus &&
                ...
                $order->setCurrentState((int) Configuration::get('LYRA_OS_REFUNDED'));
            }
    ...
    {
        if ((Configuration::get('LYRA_SEND_SHIP_DATA') === 'True') || (Configuration::get('LYRA_FFIN_ENABLED') === 'True') || (Configuration::get('LYRA_ONEY34_ENABLED') === 'True')) {
    ...
    public function createMessage($order, $response)
    {
        ...
        if ((Configuration::get('LYRA_ENABLE_CUST_MSG') === 'True') && version_compare(_PS_VERSION_, '1.7.1.2', '>=')) {
        ...
        if ($response->get('card_brand') === 'MULTI') {
            ...
            if ($response->get('operation_type') === 'CREDIT') {
                if (version_compare(_PS_VERSION_, '1.7.7', '>=')) {
                    ...
                    if (! Tools::isSubmit('doPartialRefundLyra') && ! Tools::isSubmit('doStandardRefundLyra')) {
                        $orderCurrency = new Currency((int) $order->id_currency);
                        ...
                        try {
                            if($this->lyraCreateOrderSlip($order, $amount)) {
    ...
    public function checkIdentifier($identifier, $customerEmail)
    {
        try {
            ...
            LyraTools::checkRestResult($checkIdentifierResponse);

            $cancellationDate = LyraTools::getProperty($checkIdentifierResponse['answer'], 'cancellationDate');
    ...
    {
        // LYRA_OS_REFUNDED is a final state no state change.
        if (Configuration::get('LYRA_OS_REFUNDED') == $old_state) {
            return $old_state;
        }
    ...
    private static function lyraCreateOrderSlip($order, $amount)
    {
