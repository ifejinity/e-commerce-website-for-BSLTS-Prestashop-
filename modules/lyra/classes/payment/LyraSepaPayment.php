...
class LyraSepaPayment extends AbstractLyraPayment
{
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['lyra_is_valid_sepa_identifier'] = false;
        $vars['lyra_sepa_saved_payment_mean'] = '';

        if ($this->isValidSavedAlias()) {
            $vars['lyra_is_valid_sepa_identifier'] = true;
            $customers_config = @unserialize(Configuration::get('LYRA_CUSTOMERS_CONFIG'));
            $vars['lyra_sepa_saved_payment_mean'] = isset($customers_config[$cart->id_customer][$this->name]['m']) ?
