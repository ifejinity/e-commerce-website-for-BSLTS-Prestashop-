...
class LyraStandardPayment extends AbstractLyraPayment
{
    ...
    public function getTplVars($cart)
    {
        ...
        if ($this->isEmbedded()) {
            ...
            $pub_key = $test_mode ? Configuration::get('LYRA_PUBKEY_TEST') : Configuration::get('LYRA_PUBKEY_PROD');
            $vars['lyra_std_rest_kr_public_key'] = $pub_key;

            // URL where to redirect after payment.
            $return_url = $this->context->link->getModuleLink('lyra', 'rest', array(), true);
            $vars['lyra_std_rest_return_url'] = $return_url;
            ...
            $vars['lyra_std_rest_kr_language'] = $language_iso_code;

            // kr-label-do-register.
            $vars['lyra_std_rest_kr_label_do_register'] = Configuration::get('LYRA_STD_REST_LBL_REGIST', $language['id_lang']);
            ...
            if ($pan_label = $rest_placeholders['pan'][$language['id_lang']]) {
                $vars['lyra_std_rest_kr_placeholder_pan'] = $pan_label;
            }

            // kr-placeholder-expiry.
            if ($expiry_label = $rest_placeholders['expiry'][$language['id_lang']]) {
                $vars['lyra_std_rest_kr_placeholder_expiry'] = $expiry_label;
            }

            // kr-placeholder-security-code.
            if ($cvv_label = $rest_placeholders['cvv'][$language['id_lang']]) {
                $vars['lyra_std_rest_kr_placeholder_security_code'] = $cvv_label;
            }
        }
        ...
        $vars['lyra_is_valid_std_identifier'] = false;
        ...
        if ($this->isValidSavedAlias()) {
            $vars['lyra_is_valid_std_identifier'] = true;
        ...
        if ($entry_mode === LyraTools::MODE_LOCAL_TYPE /* Card type on website. */) {
            $vars['lyra_avail_cards'] = $this->getPaymentCards();
        } elseif ($this->getEntryMode() === LyraTools::MODE_IFRAME /* Iframe mode. */) {
    ...
    public function getFormToken($cart, $use_identifier = false)
    {
        ...
        $return = false;
        try {
            $client = new LyraRest(Configuration::get('LYRA_REST_SERVER_URL'), $site_id, $key);
    ...
    {
        if ($this->getEntryMode() === LyraTools::MODE_FORM) {
            return false;
        }
    ...
    public function isEmbedded()
    {
        ...
        $embedded = array(
            LyraTools::MODE_EMBEDDED,
            LyraTools::MODE_POPIN
        );
