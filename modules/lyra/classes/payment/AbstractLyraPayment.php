...
abstract class AbstractLyraPayment
{
    ...
    {
        $this->module = Module::getInstanceByName('lyra');
    ...
    {
        return _MODULE_DIR_ . 'lyra/views/img/' . $this->logo;
    }
    ...
    public function getPaymentOption($cart)
    {
        ...
        if (file_exists(_PS_MODULE_DIR_ . 'lyra/views/img/' . $this->logo)) {
            $option->setLogo($this->getLogo());
        }


        if (! $this->hasForm()) {
            $option->setAction($this->context->link->getModuleLink('lyra', 'redirect', array(), true));
    ...
    public function prepareRequest($cart, $data = array())
    {
        ...
        // Return URL.
        $request->set('url_return', $this->context->link->getModuleLink('lyra', 'submit', array(), true));
    ...
    {
        if (isset($this->context->cookie->lyraBackendPayment)) {
            return $this->context->cookie->lyraBackendPayment;
        }
        ...
        if (isset($adminCookie->id_employee) && ! empty($adminCookie->id_employee) && Tools::getIsset('recover_cart')) {
            $this->context->cookie->lyraBackendPayment = true;
            $return = true;
        }
    ...
    private function setCartData($cart, $currency, &$lyra_request)
    {
        $products = $cart->getProducts(true);

        if (! $this->needs_cart_data && (count($products) > self::LYRA_CART_MAX_NB_PRODUCTS)) {
            return;
        }

        if (! $this->needs_cart_data && ($this->force_local_cart_data || (Configuration::get('LYRA_SEND_CART_DETAIL') !== 'True'))) {
            return;
        }
    ...
    private function setAdvancedShippingData($cart, $delivery_address, &$lyra_request)
    {
        if (Configuration::get('LYRA_SEND_SHIP_DATA') !== 'True' && ! $this->needs_shipping_method_data) {
            return;
        }

        // For Oney, some parameters must contains different data.
        $isOney34 = $this instanceof LyraOney34Payment;
        ...
        if ($cart->isVirtualCart() || ! isset($carrier_id) || ! is_array($shipping_options) || empty($shipping_options)) {
            ...
            $lyra_request->set('ship_to_delivery_company_name', preg_replace($not_allowed_chars, ' ', $company_name));
        }
    ...
    {
        return LyraTools::getDefault('LOGO_URL') . strtolower($card) . '.png';
    }
}
