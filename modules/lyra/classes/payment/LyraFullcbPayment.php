    ...
    public static function getAvailableOptions($cart)
    {
        ...
        foreach ($options as $key => $option) {
            ...
            if ((empty($min) || $amount >= $min) && (empty($max) || $amount <= $max)) {
                ...
                $option['order_amount'] = LyraTools::formatPrice($amount, $cart->id_currency, $context);
                $option['first_payment'] = LyraTools::formatPrice($first, $cart->id_currency, $context);
                $option['monthly_payment'] = LyraTools::formatPrice($payment, $cart->id_currency, $context);
                $option['total_amount'] = LyraTools::formatPrice($amount + $fees, $cart->id_currency, $context);
                $option['fees'] = LyraTools::formatPrice($fees, $cart->id_currency, $context);
