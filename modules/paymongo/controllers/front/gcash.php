 ...
 */
class PayMongoGCashModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        // Check if get request is redirect from PayMongo, otherwise init source request
        if (isset($_GET['status'])){
            if ($_GET['status']=='paid') {
            
                PrestaShopLogger::addLog("[PayMongo, GCash] Non-authoritative status paid received for Cart#". $_GET['cartId'], 1);
                ...
                PrestaShopLogger::addLog("[PayMongo, GCash] Cart#". $_GET['cartId'].' set to processing, pending confirmation by PayMongo', 1);
                ...
                PrestaShopLogger::addLog("[PayMongo, GCash] Non-authoritative unsuccessful status received for Cart#". $_GET['cartId'].' returning to payment step, no order created', 1);
            ...
            if((int)(round(($cart->getOrderTotal(true, Cart::BOTH) * 100),0))<10000){
                ...
                PrestaShopLogger::addLog("[PayMongo, GCash] Cart#".$cart->id." under minimum PHP 100 amount", 2);
                Tools::redirect($this->context->link->getPageLink(
                    'order',
                    true,
                    (int) $this->context->language->id,
