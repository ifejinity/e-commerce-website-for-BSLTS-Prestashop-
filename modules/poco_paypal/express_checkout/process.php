...
include_once(_PS_MODULE_DIR_.'poco_paypal/poco_paypal.php');
include_once(_PS_MODULE_DIR_.'poco_paypal/api/poco_paypal_lib.php');

class PoCoPaypalExpressCheckout extends PoCo_Paypal
{
	public $logs = array();
	...
	private function initParameters()
	{
		...
		if ($cart_currency->id !== $currency_module[0]['id_currency'])
		{
			$this->context->cart->id_currency = $currency_module[0]['id_currency'];
			$this->context->cart->update();
		}

		$this->context->currency = new Currency($currency_module[0]['id_currency']);
	...
	public function setExpressCheckout()
	{
		...
		// Seller informations
		$fields['USER'] = Configuration::get('POCO_PAYPAL_API_USER');
		$fields['PWD'] = Configuration::get('POCO_PAYPAL_API_PASSWORD');
		$fields['SIGNATURE'] = Configuration::get('POCO_PAYPAL_API_SIGNATURE');
	...
	{
		// Required field
		$fields['RETURNURL'] = $this->context->link->getModuleLink($this->name, 'expresscheckout');
	...
	public function redirectToCheckout($customer, $redirect = false)
	{
		...
		if ($redirect)
		{
			$link = $this->context->link->getPageLink('order', false, null, array('step' => '3'));
