...
class PoCoPayPalOrder
{
	...
	public static function getOrderById($id_order)
	{
		return Db::getInstance()->getRow(
			'SELECT * FROM `'._DB_PREFIX_.'poco_paypal_order`
	...
	public static function saveOrder($id_order, $transaction)
	{
		...
		Db::getInstance()->Execute('
			INSERT INTO `'._DB_PREFIX_.'poco_paypal_order`
			...
			VALUES ('.(int)$id_order.', \''.pSQL($transaction['id_transaction']).'\', \''.pSQL($transaction['id_invoice']).'\',
				...
				\''.(int)Configuration::get('POCO_PAYPAL_CAPTURE').'\',
				\''.pSQL($transaction['payment_date']).'\',
				\''.(int)Configuration::get('POCO_PAYPAL_PAYMENT_METHOD').'\',
	...
	public static function updateOrder($id_order, $transaction)
	{
		...
		$sql = 'UPDATE `'._DB_PREFIX_.'poco_paypal_order`
		...
		if ((int)Configuration::get('POCO_PAYPAL_SANDBOX') != 1)
