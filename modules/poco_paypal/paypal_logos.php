...
class PoCoPayPalLogos
{
	...
	{
		$file = dirname(__FILE__).'/'._POCO_PAYPAL_LOGO_XML_;
	...
	public function getLocalLogos(array $values, $iso_code)
	{
		foreach ($values as $key => $value)
			if (!is_array($value))
			{
				...
				{
					$destination = _POCO_PAYPAL_MODULE_DIRNAME_.'/img/logos/'.$iso_code.'_'.$logo[1];
					...
					if (file_exists(dirname(__FILE__).'/img/bo-cards/'.strtoupper($iso_code).'_bo_cards.png'))
						$values['BackOfficeCards'] = _MODULE_DIR_._POCO_PAYPAL_MODULE_DIRNAME_.'/img/bo-cards/'.strtoupper($iso_code).'_bo_cards.png';
					else if (file_exists(dirname(__FILE__).'/img/bo-cards/default.png'))
						$values['BackOfficeCards'] = _MODULE_DIR_._POCO_PAYPAL_MODULE_DIRNAME_.'/img/bo-cards/default.png';
				}
				// Use the local version
				else if (isset($values['Local'.$key]))
					$values['Local'.$key] = _MODULE_DIR_._POCO_PAYPAL_MODULE_DIRNAME_.$values['Local'.$key];
			}
