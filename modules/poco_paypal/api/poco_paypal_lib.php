...
include_once(_PS_MODULE_DIR_.'poco_paypal/api/poco_paypal_connect.php');
...
class PoCoPaypalLib
{
	...
	public function makeCall($host, $script, $methodName, $data, $method_version = '')
	{
		...
		$params = array(
			...
			'PWD' => Configuration::get('POCO_PAYPAL_API_PASSWORD'),
			'USER' => Configuration::get('POCO_PAYPAL_API_USER'),
			'SIGNATURE' => Configuration::get('POCO_PAYPAL_API_SIGNATURE')
		);
		...
		if (!Configuration::get('POCO_PAYPAL_DEBUG_MODE'))
			$this->_logs = array();
		...
		{
			if (!Configuration::get('POCO_PAYPAL_DEBUG_MODE') && in_array($key, $toExclude))
				continue;
	...
	{
		// Making connection
		$paypal_connect = new PoCoPayPalConnect();
