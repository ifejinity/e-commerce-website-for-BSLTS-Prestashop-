<form id="paypal_payment_form" action="{$link->getModuleLink('poco_paypal', 'expresscheckout')}" data-ajax="false" title="{l s='Pay with PayPal' mod='paypal'}" method="post" data-ajax="false">
	{if isset($smarty.get.id_product)}<input type="hidden" name="id_product" value="{$smarty.get.id_product}" />{/if}
	
	<!-- Change dynamicaly when the form is submitted -->
	<input type="hidden" name="quantity" value="1" />
	<input type="hidden" name="id_p_attr" value="{if isset($id_product_attribute)}{$id_product_attribute}{/if}" />
